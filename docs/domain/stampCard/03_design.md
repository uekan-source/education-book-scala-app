...
本部がクーポンの種類を登録し、会員は受け渡しが完了した注文 1 件につきスタンプを 1 個貯める。必要数に達したクーポンを注文で使用できる。スタンプは店舗をまたいで共通で、有効期限はクーポン 1 枚ごとに持つ。

エンティティを 3 つ追加する。既存の `User` `Order` `Payment` `Shop` `MenuItem` は変更しない。追加した 3 つが既存を参照する側になるため、参照の向きが一方向になり、既存側に列を足す必要がない。`Shop` を参照しないのはスタンプが店舗をまたいで共通だからで、`Payment` はスタンプの付与にも使用にも関与しない。

| エンティティ | テーブル | コンテキスト | 役割 |
|---|---|---|---|
| `Coupon` | `common_coupon` | `common/coupon`（新しいサブカテゴリ） | 本部が登録するクーポンの種類 |
...
- `Coupon` は `sales` を参照しない。マスタ側は自分が何枚配られたかを知らない（6章）。
- `UserCoupon` に付与日時がない。`createdAt` が兼ねる。

## 3. EntityModel

### 3.1 `Coupon` — 本部が登録するクーポンの種類
...
状態は一方向にのみ進む。通常は日次バッチ（5.5）が配布期間と比べて進め、バッチを待たず即座に反映させたいときだけ手で書き換える（5.7・5.6）。
...
- `state` を手で書き換えるのは、販売終了やキャンペーンの即日切り替えのように、バッチを待たず反映させたい場合だけである（5.7・5.6）。そのときは配布期間も同時に書き換える。片方だけを変えない。原則として `state` は配布期間から導かれる値と一致させるが、開始当日に販売終了する場合だけはその日 1 日ずれる（5.7）。
...
### 3.2 `UserCoupon` — 会員が持っているクーポン 1 枚
...
| 読み取ること | 意味 |
|---|---|
| `IS_USED` から戻る矢印がある | 使用は注文確定で確定するため、キャンセルで戻す必要がある（5.3）。他の負の状態は終端 |
| `IS_INVALIDATED` に 3 本入るが、入り方が違う | 貯め中と使用可能は、対象メニューの販売終了で落ちる（5.7）。使用済みは販売終了では落ちず、その後キャンセルされたときに初めて落ちる（5.3） |
...
`code` の符号が意味を持つ。正はまだ使える可能性がある、負は終わったことを表し、`code > 0` で「生きているクーポンか」を判定できる。正の値は 100 刻みにしているため、間に状態が増えても `150` で挿入できる。ただし `IS_USED` だけは例外で、注文がキャンセルされると `IS_AVAILABLE` に戻る（対象メニューが販売終了なら `IS_INVALIDATED`。5.3）。
...
- 会員あたり、`IS_COLLECTING` の行は最大 1 枚。1 回の受け渡しで押されるスタンプは 1 個なので、貯める先が 2 つあると決まらない。この条件は運用では守れないため、DB の一意制約で担保する。MySQL は部分ユニークインデックスを持たないので、`state` が `IS_COLLECTING` のときだけ `user_id` を返す生成列を作り、そこに一意制約を張る。5.2 のステップ 1〜5 は 1 トランザクションで行い、重複キーで落ちた側はステップ 1 からやり直す。ステップ 5 を外すと、必要スタンプ数に達しているのに `IS_COLLECTING` のまま残る行が生まれ、次の受け渡しでその行が貯め先として拾われてしまう。
...
- 使用を確定するのは注文確定時である。その注文がキャンセルされたら `IS_AVAILABLE` に戻し、`usedOrderId` と `usedAt` を None に戻す（5.3）。ただし対象メニューが販売終了になっていれば `IS_INVALIDATED` に落とし、紐づくスタンプも削除する（5.3）。未受け取りで終わった場合は戻さない。
- 使用確定は対象行をロックし、`state` が `IS_AVAILABLE` であることを確認してから更新する。二重使用を防いでいるのはこの確認だけであり、`usedOrderId` の一意制約では防げない（5.3）。
...
### 3.3 `UserCouponStamp` — 受け渡し 1 回ごとのスタンプ
...
有効期限も付与日時も持たない。スタンプ単体は失効せず、期限は親が持つ。取り消しの記録も持たない。受け渡し完了が付与のトリガーなので、押された後にキャンセルされることがない（5.3）。

型では守れない決めごと。

- `orderId` は一意。1 回の受け渡しにつきスタンプは 1 個。
- `userId` は親の `UserCoupon.userId` と常に一致する。`UserCoupon` の `userId` は作成後に変わらないため、コピーしたスタンプ側もズレない。
- 親の `UserCoupon` が `IS_EXPIRED` または `IS_INVALIDATED` になったら、紐づくスタンプは削除する。日次バッチ（5.4）、販売終了時の処理（5.7）、キャンセルで `IS_INVALIDATED` に落ちたとき（5.3）、受け渡し完了時に期限切れを検出したとき（5.2 のステップ 2）の 4 経路があり、いずれも削除する側で行う。
...
| キャッシュ | 正 | 書き換えるもの |
|---|---|---|
| `UserCoupon.state` が `IS_EXPIRED` か | `expiresAt` | 5.4 の日次バッチ（論点3） |
| `Coupon.state`（準備中 / 配布中 / 配布終了） | `distStartAt` / `distEndAt` | 5.5 の日次バッチ（論点5） |

どちらも、判定するときは `state` だけでなく正の側も確認する。`UserCoupon` の使用時は `expiresAt`（3.2）、貯め先マスタの選択時は配布期間（5.2 のステップ 3）を併せて見る。
...
付与のトリガーは `Order.Status` が `IS_DELIVERED`（受渡し完了）になることである。問題2 で追加した値をそのまま使い、`Order` 側には何も足さない。
...
ステップ 1〜5 は 1 トランザクションで行う（3.2）。

図の赤い経路、貯め先にできるマスタが 1 件も無い場合に注意する。この受け渡しにはスタンプが押されず、記録も残らない。ステップ 3 の条件は `state` が `IS_ACTIVE` かつ配布期間内かつ `requiredStampCount > 0` の 3 つで、3 つすべてを満たすマスタが 1 件も無ければこの経路に落ちる。キャンペーンの切り替え日にバッチが走る前や、配布期間に隙間がある場合に起こり得る（5.6）。
...
どちらに戻す場合も、`usedOrderId` と `usedAt` は None に戻す。注文がキャンセルされた以上そのクーポンは使われていないので、使用の記録を残さない。これにより「`usedOrderId` が埋まっているのは `IS_USED` のときだけ」という不変条件（3.2）が保たれる。`IS_INVALIDATED` に落とす場合は、紐づく `UserCouponStamp` も削除する。他の無効化の経路と揃える。

戻す先が 2 通りあるのは、使用中に対象メニューが販売終了になる場合があるためである。5.7 は `IS_COLLECTING` と `IS_AVAILABLE` だけを無効化するので、そのとき `IS_USED` だった会員クーポンは対象から外れる。無条件に `IS_AVAILABLE` へ戻すと、販売終了した商品のクーポンが「使える」状態で復活してしまう（論点4）。期限切れは日次バッチが拾い直すので自己修復するが、販売終了は一度きりの処理なので自己修復しない。
...
### 5.5 配布期間による `Coupon` の状態遷移（日次バッチ）
...
`state` は配布期間から導ける値であり、キャッシュである（論点5）。正は `distStartAt` / `distEndAt` であるため、5.2 のステップ 3 でマスタを選ぶときは `state = IS_ACTIVE` に加えて配布期間も確認する。`state` だけを信じると、終了日を過ぎたのにバッチが落ちてまだ `IS_ACTIVE` のままのマスタが選ばれ続けてしまう（論点3 の `expiresAt` と同じ理由）。

逆に、開始日当日でバッチがまだ走っていないマスタは、この確認では拾えない。`state` が `IS_PREPARING` である以上、2 つの条件を AND で結んでいる限り選ばれないためである。

### 5.6 キャンペーンを切り替えるとき

配布期間を隙間なく連続させて登録しておけば、5.5 のバッチが期日に切り替える。「バーガー無料券」の `distEndAt` の翌日から「ドリンク無料券」の `distStartAt` が始まるようにしておく。貯め中の会員はバーガー券として完成させ、次の 1 個目からドリンク券になる。

ただし切り替え日の未明からバッチが実行されるまでは、貯め先が 0 件になる。旧マスタは配布期間を過ぎており、新マスタはまだ `IS_PREPARING` のためである。5.2 のステップ 3 は `state` と配布期間の両方を確認するので、どちらも選ばれない。この時間帯に受け渡しが起きると、その 1 回分のスタンプが押されない。

この窓を許容するかどうかは、バッチの実行時刻と営業時間しだいである。深夜に実行し、その時間帯に受け渡しがないなら問題にならない。許容できないなら、5.7 と同じように配布期間と `state` を同時に書き換えて即日で切り替える。

切り替えの予定そのものを前倒しする場合も同じである。旧マスタの `distEndAt` を縮め、新マスタの `distStartAt` を早める。新マスタはまだ準備中なのですべての条件を変更できる（3.1）。ただし日付を直すだけではバッチを待つことになるので、即日で反映させたいなら `state` も同時に書き換える。

### 5.7 対象メニューを販売終了にするとき
...
`state` ごとに扱いを分ける理由は、いずれも 3.1 の決めごとを守るためである。
...
| 守ること | 破るとどうなるか |
|---|---|
| 1 トランザクションで行う | 貯め先が 0 件になる時間か 2 件並ぶ時間が生まれ、5.2 のステップ 3 で貯める先が決まらない（3.1） |
| `state` と配布期間を同時に書き換える | 正である配布期間は「配布中」、`state` は「配布終了」という食い違いが恒久的に残る。5.2 は両方を見るので実害は出ないが、配布期間だけで判定する集計や画面が、販売終了したクーポンを拾う |

`state` を手で書き換えるのは、5.5 の日次バッチを待たずに即座に反映させるためである。後継クーポンを `IS_PREPARING` のまま置くと、翌日のバッチが `IS_ACTIVE` にするまで貯め先が 0 件になる。配布期間と `state` の両方を正しい値にしておけば、次のバッチが実行されても結果は変わらない。

一時的な品切れではこの処理を行わない。対象メニューの販売終了だけが無効化のトリガーである。

## 6. 置き場所（コンテキスト）

**判断：** `Coupon` は `common/coupon` に、`UserCoupon` と `UserCouponStamp` は `sales` に置く。新しいコンテキストは作らない。

**理由：** `common` に置けるかの 3 条件が、同じ「クーポン」でも真逆に出る。

| 条件 | `Coupon`（マスタ） | `UserCoupon`（会員の 1 枚） |
|---|---|---|
| 1. 業務の操作では変わらない | ○ 本部が年に数回だけ登録する | ✗ 受け渡しのたびに変わる |
| 2. 管理者が別にいる | ○ 本部 | ✗ 会員の行為で変わる |
| 3. 他から参照されるだけ | ○ `MenuItem` を参照するだけ | ✗ `User` / `Order` を参照する側 |

決め手は条件 3 である。マスタが参照するのは同じ `common` 内の `MenuItem` だけなので依存が一方向のまま保たれるが、`UserCoupon` は `User`（`udb` コンテキスト）と `Order`（`sales`）を参照するため、`common` に置くと依存が逆流する。

`common/coupon` は 02_BURGER_DOMAIN で既に想定されていた置き場所であり、新しく考えたものではない。サブカテゴリなのでコンテキストは 4 つのまま増えず、配線も 1 セットで済む。

テーブル名はコンテキストのパスを頭に付け、単数形にする。`sales_user_coupon` と `sales_user_coupon_stamp` が隣り合い、既存の `sales_order` / `sales_order_item`（親 ＋ 子）と同じ形になる。同じ「クーポン」でも置き場所が違うことが接頭辞で読める。
...
初回は金券型（割引額のみ）だったが、端数が出る。600 円の券を 700 円のバーガーに使うと 100 円かかり、「無料券」と呼べない。要求文は「ハンバーガー 1 つが無料」なのでここが合わない。`menuItemId` を必須にすると、対象商品の価格ちょうどが引かれて端数が出ず、「注文全体から◯◯円引く」という金券型も構造的に混入しない。`discount` と組み合わせても無効な組み合わせが 1 つも生まれないため、検証が要らない。

**代償：** `common/coupon → common/menu` の参照が増える。ただし同じ `common` 内なので依存の逆流は起きない。

## 論点2：スタンプがクーポンに属する形にするか、独立させるか

**判断：** `userCouponId` を必須にし、1 個目のスタンプと同時に会員クーポンを作る。

初回は 10 個貯まった時点でクーポンを作り、そのときスタンプに参照を書き込む形（`Option`）だった。これだとスタンプが独立して存在でき、貯めている途中のスタンプはどのクーポンにも属さないため、会員が離脱すると 9 個のスタンプが永久に残る。1 個目で親を作れば、スタンプは必ず親を持ち、親が終端に達したときに `userCouponId` で引いてまとめて削除できる。

**代償：** 数える単位が `userId` から `userCouponId` に変わった。会員を起点に横断的に引きたい障害調査のためだけに、`UserCouponStamp` は `userId` を非正規化して持つ（3.3）。

## 論点3：期限切れを区分値で持つか、都度計算するか

**判断：** 区分値（`IS_EXPIRED`）を持つ。ただし正は `expiresAt`。

「比べれば分かるものは保存しない」原則からすると `expiresAt` と今日を比べれば済む。それでも持つのは、集計（配った 100 枚のうち何枚が期限切れか）と削除（対象を `state` で引く）のためである。二重管理になるので、正は `expiresAt`、`state` はキャッシュと決めた。使用時も付与時も両方を確認する。この「正を決めて全経路で確認する」形は、論点5 の `Coupon.state` でもそのまま使っている。

**代償：** 日次バッチが要る。バッチが落ちても使用は防げるが、集計と削除が遅れる。

## 論点4：対象メニューの販売終了を、期限切れと同じ状態で表すか

**判断：** `IS_EXPIRED` とは別に `IS_INVALIDATED`（`code = -3`）を持つ。

対象メニューが売られなくなると、その会員クーポンは使いようがない。`IS_AVAILABLE` のまま残すとアプリに「使える」と表示され続けるので、終端に落とす必要がある。`IS_EXPIRED` で代用しないのは原因が違うためで、同じ値にまとめると「40 枚が期限切れ」という集計に販売終了分が混ざり、クーポンの条件が厳しすぎたのか商品を切ったせいなのかが区別できなくなる。配布終了（`Coupon.IS_INACTIVE`）でも代用できない。あれは新しく配らないという意味で、配布済みの会員クーポンは期限まで使えるからである。

**代償：** 会員は自分の行為と関係なくスタンプを失う。一時的な品切れで発動しないよう、トリガーを販売終了だけに限っている。

**判断が変わる条件：** 販売終了時に代替商品のクーポンへ振り替える運用にするなら、`IS_INVALIDATED` に落とすのではなく `couponId` の付け替えになる。付け替えの履歴を残すかが新しい論点になる。

## 論点5：配布期間をマスタの属性として持つか

**判断：** `distStartAt` / `distEndAt` を持ち、`state` に `IS_PREPARING` を足して日次バッチが自動で進める。

初回は本部が `state` を手で切り替えるだけだった。これだと「いつから配るかを先に決めて登録し、その日まで見えないようにする」ができない。配布期間を持たせると将来のキャンペーンを先行登録でき、開始日が来たらバッチが自動で切り替える。`IS_PREPARING` は先行登録した状態を表すためのもので、「同時に配布中は 1 つ」の制約（3.1）の対象外なので何本でも並べられる。`state` は配布期間から導ける値なので、論点3 と同じくキャッシュとして扱い、正は配布期間に置いた。条件をいつまで直せるかもこの 3 段階で決めたので（3.1）、項目ごとに例外を覚える必要がない。

**代償：** `IS_ACTIVE` の期間が重ならないように登録する運用ルールと、バッチが 1 本増える。`distEndAt` を延ばすときは、他マスタの `distStartAt` との重複を本部が確認する必要がある。

**判断が変わる条件：** 終了日を決めずに配り続ける運用が必要になったら `distEndAt` を `Option` にする。そのときは `None` のときのバッチの扱いを別途決める。

## 論点6：無期限のクーポンを許すか

**判断：** `Coupon.validDays` と `UserCoupon.expiresAt` を `Option` にする。`None` が無期限。

誕生日クーポンのように、配った後は期限を切らずに持たせ続ける運用を想定している。`expiresAt = createdAt + validDays` という式がある以上、片方だけ `Option` にしても成立しないので、両方を変えた。`validDays = 0` で無期限を表す案は採らなかった。`createdAt.plusDays(0)` は「配られた当日に期限切れ」を意味し、無期限の対極になるためである。`requiredStampCount = 0`（0 個で配れる）と違って算術的な意味と逆をこじつけることになり、型からも値からも判別できない。将来「0 日はバグだろう」と直された瞬間に、無期限のつもりのクーポンへ期限が生まれる。

**代償：** 期限を確認する箇所すべてに `Some` / `None` の分岐が要る（3.2・5.2・5.3）。1 箇所でも書き忘れると、無期限のクーポンが期限切れ扱いされる。なお 5.4 の日次バッチだけは分岐が要らない。SQL の `NULL` 比較は真にならないため、元から対象外になる。

**判断が変わる条件：** 無期限クーポンの運用が実際に始まったら、本部の管理画面で「無期限」と分かる表示が要る。`validDays` が空欄であることが、入力漏れなのか意図的な無期限なのかを画面で区別できるようにする。
