...
`validDays` は日数であり日時ではない。マスタは何枚もの会員クーポンのテンプレートなので、絶対的な期限を持てない。`None` は無期限を表す（論点6）。

`requiredStampCount` は可変にしている。10 固定ではないため「5 個で 1 枚」のキャンペーンもマスタの登録だけで実現できる。

`distStartAt` / `distEndAt` は、いつ配るクーポンかを表す。`validDays` が会員クーポン1枚の有効期間（相対）であるのに対し、この2つはマスタ自体が配られる期間（絶対）であり、別の軸である（論点5）。`state` はこの期間から日次バッチが導く値であり、キャッシュである（論点3 の `expiresAt` と `state` の関係と同じ構図）。
...
ステップ 1〜2 は 1 トランザクションで行う。別々に行うと、貯め先が 0 件になる時間か 2 件並ぶ時間のどちらかが生まれ、第6章のステップ 3 で貯める先が決まらない（4.1）。1 トランザクションにまとめれば、どちらの状態も外から観測されない。

ステップ 1〜2 は、`state` と配布期間を必ず同時に書き換える。`state` は配布期間から導かれる値であり、正は配布期間である（8.2・論点5）。`state` だけを `IS_INACTIVE` にして `distEndAt` を先の日付のまま残すと、正に従えば配布中・`state` に従えば配布終了という食い違いが恒久的に残る。第6章のステップ 3 は両方を確認するので実害は出ないが、配布期間だけで判定する集計や画面が、販売終了したはずのクーポンを拾ってしまう。
