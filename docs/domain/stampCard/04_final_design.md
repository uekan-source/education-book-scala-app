...
- `requiredStampCount > 0` のマスタは、`IS_ACTIVE` である期間が重複しないように登録する。同時に配布中のマスタが複数あると、スタンプを貯める先が自動で選べない。マスタの登録は本部が行うので運用で担保する（論点4・論点13）。配布を終えたマスタ同士で過去の期間が重なることは、この制約の対象外である。同時に配布中になり得ないため。
- `validDays` は `Some` の場合 1 以上。`None` は無期限（論点15）。
- `distEndAt` は `distStartAt` 以降。
- `state` は `IS_PREPARING → IS_ACTIVE → IS_INACTIVE` の一方向にのみ遷移する。通常は日次バッチが配布期間と比べて自動で進める（論点13）。手動で戻すことはしない。
- `state` を手で書き換えるのは、販売終了のように即座に反映させたい場合だけである（8.1）。そのときは配布期間も同時に書き換える。片方だけを変えない。原則として `state` は配布期間から導かれる値と一致させるが、開始当日に販売終了する場合だけはその日 1 日ずれる（8.1）。
...
`IS_ACTIVE` の `distEndAt` は延長も前倒しもできる。ただし `IS_INACTIVE` になった後に書き換えても、日次バッチは `IS_INACTIVE` から戻さないため反映されない（論点13）。
...
| `Coupon.state`（準備中 / 配布中 / 配布終了） | `distStartAt` / `distEndAt` | 8.2 の日次バッチ（論点13） |
...
配布中のクーポンで `distStartAt` が今日の場合だけ `distEndAt` を今日にするのは、「`distEndAt` は `distStartAt` 以降」という決めごと（4.1）を守るためである。今朝のバッチで配布中になったクーポンを同じ日に販売終了する、という境界のケースにあたる。この日のうちは配布期間から導かれる値が「配布中」のままで `state` の `IS_INACTIVE` と食い違うが、翌日には一致する。`distStartAt` を昨日に動かせば即座に一致させられるものの、実際には今日から配り始めた記録を書き換えることになるため行わない。

準備中のクーポンだけ `distStartAt` も動かすのは、配布期間を過去に置くためである。`distEndAt` だけを縮めても `distStartAt` が未来のままでは、配布期間から導かれる値は「準備中」のままで、`state` の `IS_INACTIVE` と食い違う。両方を昨日にすれば配布期間が過去に収まり、そこから導かれる値も `IS_INACTIVE` になって一致する。準備中のクーポンはすべての条件を変更できるので（4.1）、`distStartAt` を動かすことに問題はない。

すでに `IS_INACTIVE` のクーポンにステップ 1 を行わないのは、「`IS_INACTIVE` になった後は `distEndAt` を変更できない」という決めごと（4.1）に反するためである。配布はすでに終わっているので縮める必要もない。ただしステップ 3 は行う。配布終了にしても、すでに配られた会員クーポンは期限まで使える決まりなので、その会員クーポンは販売終了によって無効化する必要がある。

ステップ 1〜2 は 1 トランザクションで行う。別々に行うと、貯め先が 0 件になる時間か 2 件並ぶ時間のどちらかが生まれ、第6章のステップ 3 で貯める先が決まらない（論点4）。1 トランザクションにまとめれば、どちらの状態も外から観測されない。

ステップ 1〜2 は、`state` と配布期間を必ず同時に書き換える。`state` は配布期間から導かれる値であり、正は配布期間である（8.2・論点13）。`state` だけを `IS_INACTIVE` にして `distEndAt` を先の日付のまま残すと、正に従えば配布中・`state` に従えば配布終了という食い違いが恒久的に残る。第6章のステップ 3 は両方を確認するので実害は出ないが、配布期間だけで判定する集計や画面が、販売終了したはずのクーポンを拾ってしまう。

`state` を手で書き換えるのは、8.2 の日次バッチを待たずに即座に反映させるためである。ステップ 2 の後継クーポンを `IS_PREPARING` のまま置くと、翌日のバッチが `IS_ACTIVE` にするまで貯め先が 0 件になる。配布期間と `state` の両方を正しい値にしておけば、次のバッチが実行されても結果は変わらない。
...
`state` は配布期間から導ける値であり、キャッシュである（論点13）。正は `distStartAt` / `distEndAt` であるため、第6章のステップ 3 でマスタを選ぶときは `state = IS_ACTIVE` に加えて配布期間も確認する。`state` だけを信じると、終了日を過ぎたのにバッチが落ちてまだ `IS_ACTIVE` のままのマスタが選ばれ続けてしまう（論点3 の `expiresAt` と同じ理由）。

逆に、開始日当日でバッチがまだ走っていないマスタは、この確認では拾えない。`state` が `IS_PREPARING` である以上、2 つの条件を AND で結んでいる限り選ばれないためである。この窓の扱いは論点4 で述べる。
...
判断：`requiredStampCount > 0` のマスタは、同時に 1 つだけ配布中にする（運用ルール）。配布期間を導入した後は、`IS_ACTIVE` である期間が重ならないように登録することでこれを担保する（論点13）。
...
キャンペーンの切り替えは、配布期間を隙間なく連続させて登録しておけば日次バッチ（8.2）が期日に切り替える。「バーガー無料券」の `distEndAt` の翌日から「ドリンク無料券」の `distStartAt` が始まるようにしておく。貯め中の会員はバーガー券として完成させ、次の 1 個目からドリンク券になる。
...
一方、開始日当日にバッチがまだ走っていない場合は、この確認では救えない。`state` が `IS_PREPARING` である以上、配布期間を併せて見ても選ばれないためである。2 つの条件を AND で結んでいるので、条件を足すほど厳しくなり、緩くはならない。この窓は論点4 で扱う。即日で切り替えたい場合は、8.1 と同じように配布期間と `state` を同時に書き換えてバッチを待たない。
...
代償：`requiredStampCount > 0` のマスタは、`IS_ACTIVE` である期間が重ならないように登録する運用ルールが増える（論点4 の「同時に配布中は1つ」と同じ性質の制約）。バッチが1本増える。`distEndAt` を延長すると、後から登録された別マスタの配布期間と重なる可能性があるため、延長時は他マスタの `distStartAt` との重複を本部が確認する必要がある。
